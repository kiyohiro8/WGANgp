...
def generator_3_SA(latent_dim, image_shape, num_res_blocks, base_name):
    ...
    out = Conv2D(3, kernel_size=3, strides=1, padding="same", kernel_initializer=initializer, activation="tanh",
               use_bias=False,name=base_name + "_conv4")(x)
    model = Model(in_x, out, name=base_name)
    return model

def generator_3(latent_dim, image_shape, num_res_blocks, base_name):
    initializer = TruncatedNormal(mean=0, stddev=0.2, seed=42)
    in_x = Input(shape=(latent_dim,))
...
def generator_2(latent_dim, image_shape, num_res_blocks, base_name):
    initializer = TruncatedNormal(mean=0, stddev=0.2, seed=42)
    in_x = Input(shape=(latent_dim,))
...
def discriminator_SA(input_shape, base_name, num_res_blocks=0,is_D=True, use_res=False):
...
def discriminator_2(input_shape, base_name, num_res_blocks=0,is_D=True, use_res=False):
    initializer_d = TruncatedNormal(mean=0, stddev=0.1, seed=42)
